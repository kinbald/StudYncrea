<?php
    ...
    class UsersModel extends Model
    {
        ...
        public function registerUser($email, $password, $name, $surname, $role, $class, $avatar = '../avatars/default.png')
        {
            if ($this->checkUserExist($email) === FALSE)
            {
                ...
                $id = $id === FALSE ? 1 : $id + 1;
                $sql = "
                    INSERT INTO " . static::$table . " (id_user, email, password_user, name_user, surname_user, role, token, id_class, url_avatar, warnings_user)
                    VALUES (:id, :email, :password_user, :name_user, :surname_user, :role, :token, :class, :url_avatar, 0)
                ";
                $password = password_hash($password, PASSWORD_DEFAULT);
                $token = $this->generateToken(35);
                $param = [':id' => $id, ':email' => $email, ':password_user' => $password, ':name_user' => $name, ':surname_user' => $surname, ':role' => $role, ':token' => $token, ':class' => $class, ':url_avatar' => $avatar];
        ...
        public function updateAvatar($id_user, $url)
        {
            $sql = "UPDATE " . static::$table . " SET url_avatar='" . $url . "' WHERE " . $this->getIdName() . "=?";
            return $this->executeReq($sql, [$id_user], 0);
        }
